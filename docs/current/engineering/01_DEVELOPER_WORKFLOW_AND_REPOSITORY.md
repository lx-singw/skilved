...
25 September B03 update: add `pnpm catalogue:test` for the source/publication package. It starts isolated Auth/Firestore emulators and requires Java 21 and Firebase CLI 15.22.3 on PATH. See [runtime commands and configuration](15_SOURCE_PUBLICATION_RUNTIME.md). Root `web:build` now compiles shared contracts before Next.js. Direct package/runtime invocation must also build those contracts first; this matters on a clean checkout. Public APIs are default-disabled and no cloud settings are needed for selected production boundary tests.
